  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <div className="min-w-0">
        ...
        {hint && <p className="mt-0.5 text-[13px] text-ink-soft dark:text-zinc-500">{hint}</p>}
      </div>
      <div className="flex shrink-0 items-center gap-3">
        <button type="button" className={btn} onClick={() => onChange(value - 1)} disabled={value <= min} aria-label="הפחת">
        ...
        <button type="button" className={btn} onClick={() => onChange(value + 1)} disabled={value >= max} aria-label="הוסף">
          <svg viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" className="h-5 w-5" strokeLinecap="round">
